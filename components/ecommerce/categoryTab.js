...
function CategoryTab() {
    ...
    const catP1 = async () => {
        const request = await fetch(`${server}/static/product.json`);
        const allProducts = await request.json();
        const cat1Item = allProducts.filter((item) => item.category == "meter-seals");
        setCat1(cat1Item);
        setActive("2");
    };
    ...
    const catP2 = async () => {
        const request = await fetch(`${server}/static/product.json`);
        const allProducts = await request.json();
        const cat2Item = allProducts.filter((item) => item.category == "pull-up-seals");
        setCat2(cat2Item);
        setActive("3");
    };
    const catP3 = async () => {
        const request = await fetch(`${server}/static/product.json`);
        const allProducts = await request.json();
        const cat3Item = allProducts.filter((item) => item.category == "high-security-seals");
        setCat3(cat3Item);
        setActive("4");
    };
